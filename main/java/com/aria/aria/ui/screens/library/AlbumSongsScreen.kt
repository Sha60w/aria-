...
fun AlbumSongsScreen(
    ...
    val albumSongs = remember(songs, album, artist) {
        songs.filter { song ->
            (song.album ?: "").equals(album, ignoreCase = false) &&
                (song.artist ?: "").equals(artist, ignoreCase = false)
        }
    }
                    ...
                    Column(modifier = Modifier.padding(16.dp)) {
                        Box(
                            modifier = Modifier
                                .fillMaxWidth()
                                .clickable {
player.playSongInSubsetQueue(albumSongs, song, autoplay = true)
                                }
                        ) {
